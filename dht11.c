...
int main(void){
   ...
   time(&t);
	...
	if((fdLocalRestore = open("home/Data/HumTemp.dat",O_RDWR|O_APPEND|S_IRWXG)) == -1){
	...
	while(1){
		if(dht11_read_val()){
			currentTime = ctime(&t);	
			sprintf(Data,"Hum:%s%sTemp:%s%sCurTim:%s%s",dht11_val[0],"\t",dht11_val[2],"\t",currentTime,"\n");	
			if(!(write_byte = write(fdLocatRestore,Data,strlen))){
