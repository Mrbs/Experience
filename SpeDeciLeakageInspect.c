...
#define DecisionTrue	1
#define DecisionFalse	0
#define SusEnable	1
#define SusDisable	0
#define PeakFound	1
#define PeakFoundNotYet	0
#define	EnableSD	1
#define	DisableSD	0
...
{
    printf("In SpeculativeDecision Function\n");
    ...
    printf("CommonePoint:%0.4lf\n",CommonePoint);
}

void Commone_Point(double FR,char *Flag,int *Counter,int *Peak,double * retSD)
{
    printf("In Commone_Point Function\n");
    double foundCommonePoint;
    printf("Initialia foundCommonePoint: %0.4lf\n",foundCommonePoint);
    ...
    *(Flag + Flag_byte) = 1 << Flag_bit;
    ...
    if(((*Peak) < 300) && (RankPeak == PeakFoundNotYet)) {
        printf("Deactivated! Cause Peak(%d) < 300\n",*Peak);
	printf("Peak : %d\n",*Peak);
	SD = DisableSD;
    } 

    if(((*Peak) > 300) && (foundCommonePoint > CommonePoint)) {
        printf("Activated\n");
        *retSD = foundCommonePoint;
        printf("foundCommonePoint: %0.4lf\n",foundCommonePoint);
...
int main(int argc,char* argv)
{
	...
	fd_Plc = open_port(fd_Plc,1,"/dev/ttyUSB2");
	...
	*/
    ...
    if((Suspicous) && (instrate == 0.0000)) {
	printf("In make decision for False\n");
        pinMode(ReturnValve,OUTPUT);
        digitalWrite(ReturnValve,HIGH);
        ...
        pinMode(SupplyValve,OUTPUT);
        digitalWrite(SupplyValve,HIGH);
        printf("Open Supply Valve\n");

        printf("Instant fluid after speculativeDecisoin %0.4lf\n",LastInstrate);
        printf("Expanding tolerance should be by 0.005\n");
        Tolerance += 0.010;
	printf("In main Function Tolerance:%0.4lf\n",Tolerance);
	Suspicous = SusDisable;
	DecisionFlag = DecisionFalse;
    }

    if((Suspicous) && (instrate > 0.1000)){
        pinMode(SupplyValve,OUTPUT);
        ...
        Tolerance -= 0.005;
	printf("In shut supplyvalve Tolerance:%0.4lf\n",Tolerance);
    }
	//	sleep(2);
	}
