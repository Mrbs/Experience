...
int main(int argc,char* argv)
{
	...
	while(1){
            ...
            serial_send(fd,"HM",2);
    			...
    			serial_read(fd,buf,200,3000);  
                	printf("the buf is :%s\n",buf);  
			buf_len = (int)strlen(buf);
			printf("strlen: %d(buf)\n",(int)strlen(buf));
			/*
			for(round = 0;round < 63;round++){
                        printf("%02x\t",buf[round]);
                }*/

			if(((buf_len == 45) && (buf[0] == '1') )){
				printf("That's for me\n");
				//1 Hum:83 Temp:42 CT:Thu Feb 
				//insert into wl_th (id,temp,hum,tm) values (1,22,24,now())
				temp[0] = buf[14];
				temp[1] = buf[15];
				hum[0] = buf[6];
				hum[1] = buf[7];
				printf("%s\n",temp);
				printf("%s\n",hum);
				sprintf(mysqlInsert,"insert into wl_th (id,temp,hum,tm) values (%d%s %s %s %s %s %s %s",id,",",temp,",",hum,",","now())","\n");
				printf("INSERT %s\n",mysqlInsert);
				int  readfile =  open("/home/App/wl.txt",O_WRONLY|O_TRUNC|O_CREAT,S_IRWXU);
        			write(readfile,mysqlInsert,strlen(mysqlInsert));
        			close(readfile);
        			bzero(mysqlInsert,100);
			}

		if(( buf_len > 165 )) { 
                                printf("That's for HeatMeter\n");
                                int  readfile =  open("/home/App/query.txt",O_WRONLY|O_TRUNC|O_CREAT,S_IRWXU);
                                write(readfile,buf,strlen(buf));
                                close(readfile);
                                bzero(buf,200);
                        }  
