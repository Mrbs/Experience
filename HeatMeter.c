...
int main(int argc,char* argv)
{
	...
	char  mysqlInsert[100];
	 ...
	 mysql_init(&my_connection);
        if(!mysql_real_connect(&my_connection,"localhost","root","root","tests",0,NULL,0))
                {
	...
	int return_temp_point_low = buf[55] % 16;
        ...
        double return_temp_point = (double)(supply_temp_point_low + supply_temp_point_ten * 10) / 100;
	...
	printf("Return temperature:%.2lf C\n",return_temp_result);
        //printf("%02x\n",buf[56]);
	sprintf(mysqlInsert, "insert into HeatMeter (id,coquan,heatquan,heatpower,instaflu,accumflu,supptemp,returtemp,Tm) values(%d %s %.2lf %s %.4lf %s %.2lf %s %.4lf  %s %.2lf %s %.2lf %s %.2lf %s %s", id,",",coolquan,",",heatquan,",",heatpower,",",instrate,",",accuflui,",",supply_temp_result,",",return_temp_result,",","now())");
